...
METHODS = ["pchip", "zero", "slinear", "quadratic", "cubic", "linear", "linear_interpolation_gpu"]
...
class Aligner:
    ...
    def run(self, n_iterations: int = None):
        ...
        for n_signal, y in enumerate(self.array):
            self.shift_opt[n_signal], self.scale_opt[n_signal] = self.compute_cuda(y)
...
#===========GPT4 RESULS============#
    ...
    def linear_interpolation_gpu(x, y, xi):
        """
        Simple linear interpolation on the GPU using CuPy.
        
        Parameters:
        - x (cp.ndarray): Known x values (1D).
        - y (cp.ndarray): Known y values, corresponding to x (1D).
        - xi (cp.ndarray): The x-coordinates to interpolate at (1D).
    
        Returns:
        - yi (cp.ndarray): Interpolated values at xi.
        """
        # Allocate output array for interpolated values
        yi = cp.zeros_like(xi)
        
        # Assuming x is sorted
        idx = cp.searchsorted(x, xi) - 1
        idx[idx < 0] = 0
        idx[idx > x.size - 2] = x.size - 2  # Clamp index
        ...
        yi = y[idx] * (1 - weight) + y[idx + 1] * weight
        
        return yi

    
    def compute_cuda(self, y):
        # Ensure all data is on the GPU
        x_gpu = cp.asarray(self.x)
        y_gpu = cp.asarray(y)
        corr_sig_x_gpu = cp.asarray(self._corr_sig_x)
        corr_sig_y_gpu = cp.asarray(self._corr_sig_y)
        search_space_gpu = cp.asarray(self._search_space)
    
        _shift = cp.asarray(self.shift_range.copy())
        _scale = cp.asarray(self._scale_range.copy())
    
        scale_opt = cp.float32(0.0)
        shift_opt = cp.float32(1.0)
    
        for n_iter in range(self.n_iterations):
            scale_grid = _scale[0] + search_space_gpu[:, (n_iter * 2) - 2] * cp.diff(_scale)
            shift_grid = _shift[0] + search_space_gpu[:, (n_iter * 2) + 1] * cp.diff(_shift)
    
            # Temporarily assuming linear interpolation for demonstration purposes
            # You'll need to replace this with a GPU-compatible interpolation
            temp = linear_interpolation_gpu(corr_sig_x_gpu, y_gpu, scale_grid, shift_grid)
    
            # Similar to np.nan_to_num, use Cupy's equivalent function if dealing with nans
            temp = cp.nan_to_num(temp)
    
            # Determining the best position needs to be adapted for use with CuPy
            i_max = cp.dot(temp, corr_sig_y_gpu).argmax()
    
            scale_opt = scale_grid[i_max]
            shift_opt = shift_grid[i_max]
    
            # Adjust grid for next iteration
            _scale = scale_opt + (cp.array([-0.5, 0.5]) * cp.diff(_scale) * self._reduce_range_factor)
            _shift = shift_opt + (cp.array([-0.5, 0.5]) * cp.diff(_shift) * self._reduce_range_factor)
    
        return shift_opt.get(), scale_opt.get()
