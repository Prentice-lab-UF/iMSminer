...
class Aligner_GPU:
    ...
    def run_batch(self, n_iterations: int = None):
        ...
        print(f"Processed {self.array.shape[0]} signals in {
              time.time() - t_start} seconds")
        self._computed = True
        ...
        if value:
            ...
            while count_left < count_rep:
                ...
                if count_left == count_rep:
                    # +2 to rely more on symmetry for better peaks at deviating regions
                    value_left += (count_rep-1+2)
...
def get_chunk_ms_info_inhomogeneous(p, chunk_start, chunk_size, max_mz, min_mz, mz_RP, RP, dist, rp_factor):
    ...
    for idx, (x, y, z) in enumerate(p.coordinates[chunk_start:chunk_start+chunk_size]):
        bin_spectrum = np.zeros(
            [int((max_mz-min_mz)/(mz_RP/RP) * rp_factor)+10])
        index = chunk_start + idx
        mzs, intensities = p.getspectrum(index)
...
def chunk_prep_inhomogeneous(p, min_mz, max_mz, mz_RP, RP, dist, percent_RAM=5):
...
def integrate_peak(chunk_ms_dict, p2, p2_width, chunk_0_mz_peaks, method="peak_width", remove_duplicates=False):
    ...
    p2_width : array of float
        Peak regions computed by np.peak_widths
    chunk_0_mz_peaks : array of float
    ...
    if method == "peak_width":
        ...
        peak_area_df_temp = pd.DataFrame(np.zeros(
            ...
            (len(p2_new_temp), intensity_df_temp.shape[1]-1)), index=chunk_0_mz_peaks)

        for column in intensity_df_temp.columns[1:]:
            peak_area = [np.trapz(chunk_ms_dict["I"][column][np.arange(int(row[0]), int(row[1]) + 1)], chunk_ms_dict["mz"][np.arange(
                int(row[0]), int(row[1]) + 1)]-chunk_ms_dict["mz"][np.arange(int(row[0])-1, int(row[1]))]) for row in peak_width]
...
def get_p2(p, loq, lwr, upr, rel_height, chunk_size_base, num_chunks, chunk_start, remainder, dist, noise=0, method="point"):
    ...
    for i in range(num_chunks):
        ...
        print(f"Time used for running chunk {i} with a size of {
              chunk_size_temp}: {time.time()-start_time}")
        gc.collect()
    ...
    for k in range(matrix.shape[1]-2):
        ...
        try:
            color_img = cv2.applyColorMap(
                matrix[matrix.columns[k]].to_numpy().astype(np.uint8), cv2.COLORMAP_RAINBOW)
        except:
            color_img = np.stack(
                [matrix[matrix.columns[k]].to_numpy().astype(np.uint8)] * 3, axis=-1)

        for row, coord in matrix[matrix.columns[[-2, -1]]].iterrows():
            row = int(row)
            img_array_1c[k, coord['x']-x_min, coord['y'] -
                         y_min, 0:1] = matrix[matrix.columns[k]][row]
            img_array_3c[k, coord['x']-x_min, coord['y'] -
                         y_min, 0:3] = color_img[row, 0]  # 0:2 RGB
...
#IPython-based ROI selection; for notebooks on remote networks 
class bbox_select():
