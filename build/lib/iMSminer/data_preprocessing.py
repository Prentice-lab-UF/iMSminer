...
from assorted_functions import get_chunk_ms_info, get_chunk_ms_info2, get_chunk_ms_info_inhomogeneous, chunk_prep, chunk_prep_inhomogeneous, integrate_peak, get_spectrum, Aligner_CPU, str2bool
import gc
import time
import numpy as np
import pandas as pd
...
def prompt_func(self, attr_name):
    prompts = {
        'dist': lambda: int(input("Enter the minimum number of data points between near-isobars: ")),
        'loq': lambda: float(input("What is your limit of quantification? Enter a coefficient k such that LOQ = k * noise: ")),
        'pp_dataset': lambda: input("Enter dataset name (without '') to perform peak picking on: "),
        'lwr': lambda: float(input("Enter a lower m/z bound of noise: ")),
        'upr': lambda: float(input("Enter an upper m/z bound of noise: ")),
        'noise': lambda: float(input("Specify a noise level. Enter a number noise such that LOQ = k * noise: ")),
        'z_score': lambda: float(input("Enter a z_score bound for noise classification: ")),
...
class Preprocess:
    ...
    def __init__(
        ...
        self.directory = input("Enter the directory of imzML files to process: ")
        self.data_dir = input("Enter a directory path for saving preprocessed data: ")
    ...
    def check_attributes(self):
        attributes = {
            'dist': lambda: int(input("Enter the minimum number of data points required to discern closely-spaced peaks: ")),
            'loq': lambda: float(input("Specify an intensity threshold for peak picking. Enter a coefficient k such that threshold = k * noise: ")),
            'pp_dataset': lambda: input("Enter dataset name (without '') to perform peak picking on: ") if self.datasets.shape[0] > 1 else self.datasets[0],
            'lwr': lambda: float(input("Enter a lower m/z bound of noise: ")) if self.pp_method == "point" else None,
            'upr': lambda: float(input("Enter an upper m/z bound of noise: ")) if self.pp_method == "point" else None,
            'noise': lambda: float(input("Specify a noise level. Enter a number noise such that LOQ = k * noise: ")) if self.pp_method == "specify_noise" else None,
            'z_score': lambda: float(input("Enter a z_score bound (number of standard deviations) for noise classification: ")) if self.pp_method in ["automatic", "binning_even", "binning_regression"] else None,
        }
        if self.pp_method in ["binning_even", "binning_regression"]:
            attributes.update({
                'RP': lambda: float(input("Specify the resolving power (FWHM): ")),
                'mz_RP': lambda: float(input("Specify the m/z at which resolving power was calculated: ")),
                'rp_factor': lambda: float(input("Scale number of m/z bins up or down by a coefficient k (i.e., k * (m/z range of MS analysis) * FWHM / (m/z) = number of bins): "))
            })
        return attributes
    ...
    def peak_pick(
        ...
        datasets = np.asarray(datasets)[(np.char.find(datasets, "imzML") != -1)]
    ...
    def run(
        ...
        for dataset in self.datasets:
            ...
            for i in range(num_chunks):
                ...
                chunk_ms_dict['mz'], chunk_ms_dict['I'] = self.peak_alignment_func(mz=chunk_ms_dict['mz'], intensity=chunk_ms_dict['I'])

                # chunk 0 settings
                if i == 0:
                    peaks_df = pd.DataFrame(np.zeros(
                        (len(self.p2), 0)), index=chunk_ms_dict['mz'][self.p2])
                
                if not self.inhomogeneous:
                    peak_area_df_temp = integrate_peak(chunk_ms_dict, self.p2, self.p2_width, self.dist, chunk_ms_dict['mz'][self.p2],
                                                       integrate_method=integrate_method)
                else:
                    peak_area_df_temp = pd.DataFrame(
                        chunk_ms_dict["I"].T[self.p2, :], index=chunk_ms_dict['mz'][self.p2])
    ...
    def peak_pick_func(
        ...
        try:
            ...
            self.num_chunks, self.chunk_size_base, self.chunk_start, self.remainder = chunk_prep(
                self.p, self.percent_RAM)
            if self.pp_method == "point":
                ...
                for i in range(self.num_chunks):
                    ...
                    intensities_chunk = self.p.get_intensity_chunk(chunk_start=self.chunk_start, chunk_end=(
                        self.chunk_start+chunk_size_temp), index_stop=index_stop)
                    mzs, intensities_chunk = self.peak_alignment_func(mz=mzs, intensity=intensities_chunk)
                    
                    try:
                        if self.plot_aligned_peak:
                            plt.style.use("classic")
                            for spectrum_index in range(0, intensities_chunk.shape[0], int(intensities_chunk.shape[0]/self.plot_num_peaks)):
                                plt.plot(
                                    intensities_chunk[spectrum_index][self.p2[self.index_peak_plot]-20:self.p2[self.index_peak_plot]+20])
                                plt.plot(
                                    20, intensities_chunk[spectrum_index][self.p2[self.index_peak_plot]], "x")
                                plt.hlines(*np.concatenate(([intensities_chunk[spectrum_index][self.p2[self.index_peak_plot]]/2],
                                                            np.asarray(self.p2_width)[2:, self.index_peak_plot]-self.p2[self.index_peak_plot]+20),
                                                           axis=0), color="C2")  
                            plt.show()
                ...
                noise = np.mean(noise_array[abs(scipy.stats.zscore(noise_array))<3]) / self.num_chunks
                self.noise = noise
                self.noise_array = noise_array / self.num_chunks  
                self.baseline_subtraction()
                    
                p2 = find_peaks(x=self.avg_spectrum, height=self.loq * noise, 
                                distance=self.dist)
                p2 = p2[0]
            ...
            get_spectrum(output_filepath=f'{self.data_dir}/{self.pp_dataset[:-6]}_avg_spectrum.html',
                         mzs=self.mzs, avg_intensity=self.avg_spectrum, p2=self.p2)
    
        except Exception:
            self.baseline_subtract = False
            self.pp_method = input("Select binning method: `binning_even` or `binning_regression`. Use `binning_regression`, which requires simple linear regression equation from original data, for more optimal mass resolution. ")
            self.check_attributes()
            ...
            if self.pp_method == "binning_even":
                ...
                while not resolution_exit:
                    n_bins = int((max_mz-min_mz)/(self.mz_RP/self.RP) * self.rp_factor) + 1
                    self.n_bins = n_bins
                    ...
                    for i in range(num_chunks):
                        ...
                        chunk_ms_dict = get_chunk_ms_info_inhomogeneous(
                            self.p, chunk_start, chunk_size_temp, self.max_mz, self.min_mz, self.mz_RP, self.RP, self.dist, self.rp_factor)
                        self.mzs, chunk_ms_dict["I"] = self.peak_alignment_func(mz=self.mzs, intensity=chunk_ms_dict["I"])
                    ...
                    self.noise = np.std(
                        avg_spectrum[abs(scipy.stats.zscore(avg_spectrum)) < self.z_score])
                ...
                try:
                    ...
                    regression_eq = input("Specify a simple linear regression equation of intercept + coefficient. Enter: intercept{one space}coefficient ")
                    regression_eq = regression_eq.split(" ")
                    regression_eq = np.asarray(regression_eq).astype(np.float32)
                ...
                while not resolution_exit:
                    ...
                    for i in range(len(self.p.coordinates)):
                        ...
                        intensity_index = np.digitize(spectrum_i[0], self.mzs) - 1
                        intensity_array = np.zeros(n_bins)
                        intensity_array[intensity_index] += spectrum_i[1]
                        self.mzs, intensity_array = self.peak_alignment_func(mz=self.mzs, intensity=intensity_array)
                    ...
                    self.FWHM = [self.mzs[self.p2[i]]/(self.mzs[np.floor(row[1]).astype(int)]*
                                (1-scale_factor[i][1])+self.mzs[np.ceil(row[1]).astype(int)]*
                                (scale_factor[i][1])-(self.mzs[np.floor(row[0]).astype(int)]*
                                (1-scale_factor[i][0])+self.mzs[np.ceil(row[0]).astype(int)]*
                                (scale_factor[i][0]))) for i,row in enumerate(p2_half_width)]
            
                    avg_spectrum = np.zeros(n_bins)
                    
                    rp_predicted = self.regression_eq[0] + self.regression_eq[1] * np.linspace(min_mz,max_mz,n_bins)
                    rp_predicted = rp_predicted / np.sum(rp_predicted)
                    
                    self.mzs = np.interp(np.linspace(0,1,n_bins), np.cumsum(rp_predicted), np.linspace(min_mz, max_mz, n_bins))
    
                    for i in range(len(self.p.coordinates)):
                        spectrum_i = self.p.getspectrum(i)
                        spectrum_i[0], spectrum_i[1] = self.peak_alignment_func(mz=spectrum_i[0], intensity=spectrum_i[1])
                        intensity_index = np.digitize(spectrum_i[0], self.mzs) - 1
                ...
                print("Unrecognized method. Choose an option from `automatic`, `specify_noise`, or `point` if raw imzML, and `binning_even` or `binning_regression` if reduced imzML.")
                return

    def peak_alignment_func(
                  self,  
                  mz,
                  intensity
                  ):
        ...
        if self.peak_alignment:
            ...
            if self.gpu:
                ...
                if self.align_reduce:
                    reduced_index = [
                        np.arange(val-self.reduce_halfwidth, val + self.reduce_halfwidth, 1) for val in self.p2]
                    reduced_index = np.concatenate(reduced_index)
                    reduced_index = reduced_index[reduced_index <
                                                  self.mzs.shape[0]-1]
                    alinger = Aligner_GPU(x=mz[reduced_index],
                                          peaks=mz[self.p2][self.avg_spectrum[self.p2] > self.align_threshold*self.noise],
                                          array=intensity[:,
                                                                   reduced_index],
                                          method="gpu_linear",
                                          align_by_index=True,
                ...
                if self.align_reduce:
                    ...
                    reduced_index = reduced_index[reduced_index <
                                                  self.mzs.shape[0]-1]
                    alinger = Aligner_CPU(x=mz[reduced_index],
                                          peaks=mz[self.p2][self.avg_spectrum[self.p2] > self.align_threshold*self.noise],
                                          array=intensity[:,
                                                                   reduced_index],
                                          method="linear",
    ...
    def get_p2(
        ...
        for i in range(self.num_chunks):
            ...
            if self.pp_method == "point":
                noise_bin = np.std(np.asarray(intensities_chunk)[
                                   :, np.logical_and(mzs >= self.lwr, mzs <= self.upr)], axis=0)
        ...
        self.avg_spectrum = avg_spectrum
        self.mzs = mzs
        p2, info = find_peaks(x=avg_spectrum, height=self.loq*self.noise, distance=self.dist)
            ...
            if self.baseline_subtract:
                if self.baseline_method == "regression":
                    ...
                    while not baseline_exit:
                        baseline_index = abs(scipy.stats.zscore(self.noise_array))<2
                        degree = int(input("Enter degree of regression for baseline computation: "))
                        poly = PolynomialFeatures(degree=degree, include_bias=False)
                        poly_features = poly.fit_transform(
                            self.mzs[baseline_index].reshape(-1, 1))
                        poly_reg_model = LinearRegression()
                        poly_reg_model.fit(poly_features, self.noise_array[baseline_index])
                        R2 = poly_reg_model.score(poly_features, self.noise_array[baseline_index])
                        ...
                        ax.scatter(self.mzs,
                                    baseline_pred, color="red")
                        plt.plot(self.mzs[baseline_index], self.noise_array[baseline_index], alpha=0.3, color="black")
                        ...
                        baseline_exit = input("Satisfied with baseline subtraction? Enter (yes / no): ")
                        baseline_exit = str2bool(baseline_exit)
                    self.baseline_pred = baseline_pred
                elif self.baseline_method == "noise":
                    baseline_factor = float(input("Enter a coefficient k for baseline, such that baseline = k * noise: "))
